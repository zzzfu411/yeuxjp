...
test("reference modal navigation hook owns selected index and arrow-key behavior", () => {
  ...
  assert.match(source, /shouldHandleGlobalShortcutEvent\(event\)/)
...
test("GrammarReferenceControls owns grammar search, level state, and modal navigation", () => {
  ...
  assert.match(source, /<GrammarPointList points=\{pagePoints\} activeLevel=\{activeLevel\} onOpen=\{index => openAt\(pageStart \+ index\)\} \/>/)
  assert.match(source, /import \{ Search, SearchX \} from "lucide-react"/)
  assert.match(source, /currentPoints\.length \? \(/)
  assert.match(source, /role="status"/)
  assert.match(source, /没有找到匹配语法/)
  assert.match(source, /试试更短的关键词，或切换其他等级。/)
  assert.match(source, /<GrammarFocusModal/)
  assert.match(source, /<SpeechSettingsBar collapsible className="max-w-3xl mx-auto" \/>/)
...
test("grammar list and focus modal own grammar presentation copy", () => {
  ...
  assert.match(list, /aria-haspopup="dialog"/)
...
test("semantics and pragmatics reference pages keep static lists in server components", () => {
  ...
  assert.match(semanticsPage, /<ReferenceCatalog/)
  ...
  assert.match(pragmaticsPage, /description: scenario\.situation/)
  assert.match(pragmaticsModal, /场景：\{scenario\.situation\}/)
  assert.match(pragmaticsModal, /使用背景/)
  assert.match(pragmaticsModal, /表达说明/)
...
test("legacy reference query URLs redirect to static detail routes", () => {
  ...
  assert.match(source, /"use client"/)
  assert.match(source, /useSearchParams\(\)/)
  assert.match(source, /from "@\/lib\/pwa-navigation"/)
  assert.match(source, /shouldUsePwaDocumentNavigationForHref/)
  assert.match(source, /searchParams\.get\("item"\)/)
  assert.match(source, /validIds\.includes\(itemId\)/)
  assert.match(source, /const targetHref = `\$\{basePath\}\/\$\{encodeURIComponent\(itemId\)\}`/)
  assert.match(source, /href: targetHref/)
  assert.match(source, /isOnline: navigator\.onLine/)
  assert.match(source, /hasServiceWorkerController: "serviceWorker" in navigator && Boolean\(navigator\.serviceWorker\.controller\)/)
  assert.match(source, /window\.location\.replace\(targetHref\)/)
  assert.match(source, /router\.replace\(targetHref\)/)
})
...
test("URL-controlled reference modal owns client-side close and arrow-key navigation", () => {
  ...
  assert.match(source, /from "@\/lib\/pwa-navigation"/)
  assert.match(source, /const navigate = useCallback\(\(href: string\) =>/)
  assert.match(source, /shouldUsePwaDocumentNavigationForHref\(\{/)
  assert.match(source, /href,/)
  assert.match(source, /currentLocation: window\.location/)
  assert.match(source, /isOnline: navigator\.onLine/)
  assert.match(source, /hasServiceWorkerController: "serviceWorker" in navigator && Boolean\(navigator\.serviceWorker\.controller\)/)
  assert.match(source, /window\.location\.assign\(href\)/)
  assert.match(source, /router\.push\(href\)/)
  assert.match(source, /navigate\(closeHref\)/)
  assert.match(source, /from "@\/lib\/keyboard-shortcuts"/)
  assert.match(source, /shouldHandleGlobalShortcutEvent\(event\)/)
  assert.match(source, /event\.key === "ArrowRight"/)
  assert.match(source, /event\.preventDefault\(\)/)
  assert.match(source, /navigate\(nextHref\)/)
  assert.match(source, /event\.key === "ArrowLeft"/)
  assert.match(source, /navigate\(prevHref\)/)
  assert.match(source, /window\.addEventListener\("keydown", handleKeyDown\)/)
  assert.match(source, /window\.removeEventListener\("keydown", handleKeyDown\)/)
...
test("vocabulary page keeps card-specific flip behavior while sharing indexed navigation", () => {
  ...
  assert.match(source, /onExpand=\{index => openAt\(pageStart \+ index\)\}/)
