...
test("kana page delegates reusable data derivation to kana-page-model", () => {
  ...
  const grid = fs.readFileSync(path.join(root, "src/components/kana/kana-grid.tsx"), "utf8")
  ...
  assert.match(controls, /setKanaSet\(parsedSet \?\? "seion"\)/)
  ...
  assert.match(source, /useKanaPageData\(kanaSet, mode, onlyUnmastered, isMastered\)/)
  assert.match(hook, /from "@\/lib\/kana-page-model"/)
  assert.match(hook, /makeKanaId\(mode, romaji\)/)
  assert.match(hook, /getKanaSetData\(kanaData, kanaSet\)/)
  assert.match(hook, /filterKanaByProgress\(seion, onlyUnmastered, isModeMastered\)/)
  assert.match(hook, /getKanaRowsForData\(KANA_ROWS\.seion, visibleSeion\)/)
  assert.match(hook, /getKanaProgress\(activeData, isModeMastered\)/)
  assert.match(grid, /makeKanaId\(mode, item\.romaji\)/)
  assert.match(grid, /onToggleMastered\?\.\(selectedKanaId\)/)
