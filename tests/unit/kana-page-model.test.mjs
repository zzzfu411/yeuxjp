...
test("kana page delegates reusable data derivation to kana-page-model", () => {
  ...
  assert.match(source, /from "@\/lib\/kana-page-model"/)
  assert.match(source, /parseKanaSet\(urlSet\)/)
  assert.match(source, /getKanaSetData\(kanaData, kanaSet\)/)
  assert.match(source, /filterKanaByProgress\(list, onlyUnmastered, isMastered\)/)
  assert.match(source, /getKanaRowsForData\(rows, list\)/)
  assert.match(source, /getKanaProgress\(activeData, isMastered\)/)
