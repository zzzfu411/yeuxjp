...
function bindStack(openModals, surface) {
  return (event) => arrows.shouldHandleModalArrowNavigation(event, () => openModals.isTopOpenModal(surface))
}
...
test("arrow navigation stays live for a sole open modal", () => {
  ...
  const decide = bindStack(openModals, detail)
...
test("buried arrow handlers are inert while another modal is topmost", () => {
  ...
  const decideBuried = bindStack(openModals, detail)
  const decideTop = bindStack(openModals, speech)
  ...
  assert.equal(decideBuried(keyEvent("ArrowRight")), null)
  assert.equal(decideBuried(keyEvent("ArrowLeft")), null)
  assert.equal(decideTop(keyEvent("ArrowRight")), "ArrowRight")
  assert.equal(decideTop(keyEvent("ArrowLeft")), "ArrowLeft")
})
...
test("arrow navigation stays inert after a keyed underlay remounts under speech", () => {
  ...
  const decideBuried = bindStack(openModals, remounted)
  const decideKey = bindStack(openModals, "url-controlled-reference")
  ...
  assert.equal(decideBuried(keyEvent("ArrowRight")), null)
  assert.equal(decideBuried(keyEvent("ArrowLeft")), null)
  assert.equal(decideKey(keyEvent("ArrowRight")), null)
  assert.equal(decideKey(keyEvent("ArrowLeft")), null)
})

test("arrow navigation stays live when remounted reference stacks above speech", () => {
  const openModals = stack.createOpenModalStack()
  const detail = makeDialog("detail-1")
  const speech = makeDialog("speech")
  const remounted = makeDialog("detail-2")
  const decideTop = bindStack(openModals, remounted)
  const decideKey = bindStack(openModals, "url-controlled-reference")
  const decideBuried = bindStack(openModals, speech)
  ...
  assert.equal(decideTop(keyEvent("ArrowRight")), "ArrowRight")
  assert.equal(decideTop(keyEvent("ArrowLeft")), "ArrowLeft")
  assert.equal(decideKey(keyEvent("ArrowRight")), "ArrowRight")
  assert.equal(decideKey(keyEvent("ArrowLeft")), "ArrowLeft")
  assert.equal(decideBuried(keyEvent("ArrowRight")), null)
  assert.equal(decideBuried(keyEvent("ArrowLeft")), null)
})
