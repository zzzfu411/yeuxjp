...
test("PWA offline E2E verifies production service worker fallback without state writes", () => {
  ...
  assert.match(pwaE2e, /process\.argv\.includes\("--required"\)/)
  assert.match(pwaE2e, /E2E_PWA_REQUIRED === "1"/)
  assert.match(pwaE2e, /PWA E2E skipped: Playwright is not installed/)
  assert.match(pwaE2e, /npmCommand\(\), \["run", "build"\]/)
  assert.match(pwaE2e, /npmCommand\(\), \["run", "start"/)
