...
const modeToggle = fs.readFileSync(path.join(root, "src/components/mode-toggle.tsx"), "utf8")
...
test("PWA registers a production service worker and exposes install metadata", () => {
  ...
  assert.match(themeProvider, /document\.querySelector<HTMLMetaElement>\('meta\[name="theme-color"\]'\)/)
  assert.match(themeProvider, /resolvedTheme === "dark" \? "#000000" : "#facc15"/)
...
test("PWA offline E2E verifies visited-page cache, fallback, and local state preservation", () => {
  ...
  assert.match(pwaE2e, /\/review/)
  assert.match(pwaE2e, /seedReviewState/)
  assert.match(pwaE2e, /online review prewarm should load due local review state/)
  assert.match(pwaE2e, /\/path/)
  assert.match(pwaE2e, /path-next-learning/)
  assert.match(pwaE2e, /\/grammar\?level=N5/)
  assert.match(pwaE2e, /grammar-point-n5-wa/)
  assert.match(pwaE2e, /online grammar prewarm should load the grammar reference/)
  ...
  assert.match(pwaE2e, /offline path page should render the cached learning path/)
  assert.match(pwaE2e, /offline grammar query should render the cached grammar reference/)
  assert.match(pwaE2e, /offline review page should render due local review state/)
  assert.match(pwaE2e, /offline review session should render the seeded kana answer/)
  assert.match(pwaE2e, /review-answer-a/)
  ...
  assert.match(pwaE2e, /managedLearningBackupKeys/)
  assert.match(pwaE2e, /learningStateSentinel/)
  assert.match(pwaE2e, /persistedLearningState/)
  assert.match(pwaE2e, /offline fallback must not overwrite any managed local learning state/)
})
...
test("PWA offline E2E asserts cached learning content, not just route shells", () => {
  ...
  assert.match(pwaE2e, /GRAMMAR_DOJO/)
