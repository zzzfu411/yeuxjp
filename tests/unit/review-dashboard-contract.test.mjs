...
test("review page delegates dashboard surfaces while keeping SRS/session wiring", () => {
  ...
  assert.match(page, /const enrollKanaMissing = \(\) => setReviewSaveError\(!dashboard\.kanaEnrollMissing\.every\(\(id\) => kanaSrs\.enroll\(id\)\)\)/)
  assert.match(page, /const enrollVocabMissing = \(\) => setReviewSaveError\(!dashboard\.vocabEnrollMissing\.every\(\(id\) => vocabSrs\.enroll\(id\)\)\)/)
