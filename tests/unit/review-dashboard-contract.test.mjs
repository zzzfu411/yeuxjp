...
test("review page delegates dashboard surfaces while keeping SRS/session wiring", () => {
  ...
  assert.match(page, /const enrollKanaMissing = \(\) => setReviewSaveError\(!kanaEnrollMissing\.every\(\(id\) => kanaSrs\.enroll\(id\)\)\)/)
  assert.match(page, /const enrollVocabMissing = \(\) => setReviewSaveError\(!vocabEnrollMissing\.every\(\(id\) => vocabSrs\.enroll\(id\)\)\)/)
  ...
  assert.match(page, /startSession\(\{ deck: "today", items: todayQueue \}\)/)
  assert.match(page, /startSession\(\{ deck: "kana", ids: reviewableKanaDueIds \}\)/)
  assert.match(page, /startSession\(\{ deck: "vocab", ids: vocabSrs\.dueIds \}\)/)
  assert.match(page, /startSession\(\{ deck: "mistakes", ids: dueMistakeIds \}\)/)
