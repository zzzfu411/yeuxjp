...
test("review page delegates dashboard surfaces while keeping SRS/session wiring", () => {
  ...
  assert.match(page, /const \[mistakeSaveError, setMistakeSaveError\] = useState\(false\)/)
  assert.match(page, /const startSession = \(nextSession: ReviewSession\) => \{/)
  assert.match(page, /setMistakeSaveError\(false\)/)
  assert.match(page, /const removeMistake = \(id: string\) => setMistakeSaveError\(!mistakes\.remove\(id\)\)/)
  assert.match(page, /const clearMistakes = \(\) => setMistakeSaveError\(!mistakes\.clear\(\)\)/)
