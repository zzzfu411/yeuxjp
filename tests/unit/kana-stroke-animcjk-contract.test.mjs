...
test("useAnimCjkPlayback owns timer scheduling, speed, pause, and replay state", () => {
  ...
  assert.match(source, /const activeStrokeRef = useRef\(0\)/)
  assert.match(source, /const timersRef = useRef<ReturnType<typeof setTimeout>\[\]>\(\[\]\)/)
  assert.match(source, /const updateActiveStroke = useCallback/)
  assert.match(source, /getAnimCjkTimelineEvents\(\{ startFrom, totalStrokes, speed \}\)/)
  assert.match(source, /getAnimCjkPlaybackStartStroke\(\{ activeStroke: activeStrokeRef\.current, totalStrokes \}\)/)
  assert.match(source, /updateActiveStroke\(event\.stroke\)/)
  ...
  assert.doesNotMatch(source, /eslint-disable-next-line/)
})
...
test("KanaStrokeControls owns playback control buttons and labels", () => {
  ...
  assert.match(source, /data-testid="kana-stroke-progress"/)
  assert.match(source, /data-testid="kana-stroke-prev"/)
  assert.match(source, /aria-label="上一笔"/)
  assert.match(source, /aria-label=\{isPaused \|\| isFinished \? "播放" : "暂停"\}/)
  assert.match(source, /data-testid="kana-stroke-play-toggle"/)
  assert.match(source, /<ControlBtn onClick=\{onNext\}/)
  assert.match(source, /data-testid="kana-stroke-next"/)
  assert.match(source, /aria-label="下一笔"/)
  assert.match(source, /<ControlBtn onClick=\{onReplay\}/)
  assert.match(source, /data-testid="kana-stroke-replay"/)
  assert.match(source, /aria-label="重播"/)
  assert.match(source, /data-testid="kana-stroke-speed"/)
