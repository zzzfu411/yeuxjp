...
test("TodayReviewSession owns mixed queue data, shared prompts, and all SRS deck grading", () => {
  ...
  assert.match(source, /return kanaSrs\.grade\(current\.id/)
  assert.match(source, /return vocabSrs\.grade\(current\.id/)
  assert.match(source, /return mistakeSrs\.grade\(current\.id/)
