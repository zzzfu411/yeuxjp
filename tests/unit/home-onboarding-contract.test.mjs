...
test("home page delegates first-run profile setup to OnboardingPanel", () => {
  ...
  assert.match(page, /from "@\/lib\/path-page-model"/)
  assert.match(page, /useLearningStatus\(\)/)
  assert.match(page, /getKanaSkillStats\(kanaData, learning\.isKanaMastered\)/)
  assert.match(page, /summarizeLearnedVocabIds\(learning\.learnedVocabIds\)/)
  assert.match(page, /getRecommendedSkillId\(kanaStats, vocabStats\)/)
