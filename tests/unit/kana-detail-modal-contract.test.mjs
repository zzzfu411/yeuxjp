...
test("KanaDetailModal owns stroke, speech, mastery, and navigation controls", () => {
  ...
  assert.match(source, /className="flex min-h-full flex-col items-center justify-start .*sm:justify-center/)
  assert.match(source, /className="flex w-full flex-wrap gap-3"/)
  assert.match(source, /className="flex shrink-0 items-center justify-between border-t/)
  ...
  assert.match(source, /正在加载笔顺动画/)
  assert.match(source, /这个假名暂时没有笔顺动画/)
  assert.doesNotMatch(source, />[^<]*AnimCJK 笔顺资源/)
