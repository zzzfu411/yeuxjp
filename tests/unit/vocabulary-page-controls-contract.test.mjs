...
test("useVocabularyPageControls owns URL level and vocabulary toolbar state", () => {
  ...
  assert.match(source, /DEFAULT_VOCABULARY_LEVEL/)
  assert.match(source, /isVocabLevel/)
  assert.match(source, /const \[currentLevel, setCurrentLevel\] = useState<VocabLevel>\(DEFAULT_VOCABULARY_LEVEL\)/)
  ...
  assert.match(source, /return isVocabLevel\(value\) \? value : null/)
  assert.match(source, /const parsedLevel = parseVocabularyLevel\(urlLevel\)/)
  assert.match(source, /setCurrentLevel\(parsedLevel \?\? DEFAULT_VOCABULARY_LEVEL\)/)
  ...
  assert.match(source, /const VOCABULARY_SHORT_VIEWPORT_SCROLL_OFFSET = 96/)
  assert.match(source, /window\.innerHeight <= 420/)
  assert.match(source, /\? VOCABULARY_SHORT_VIEWPORT_SCROLL_OFFSET/)
  assert.match(source, /- scrollOffset/)
