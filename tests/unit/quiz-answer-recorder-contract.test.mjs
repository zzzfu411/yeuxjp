...
test("quiz runner delegates shared answer recording to useQuizAnswerRecorder", () => {
  ...
  assert.match(source, /selectedOption \|\| answerPendingRef\.current/)
  assert.match(source, /answerPendingRef\.current = true/)
  assert.match(source, /answerPendingRef\.current = false/)
  assert.match(source, /if \(!result\) \{\s*answerPendingRef\.current = false\s*setSaveError\(true\)\s*return\s*\}/)
