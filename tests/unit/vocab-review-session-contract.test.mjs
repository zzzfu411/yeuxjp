...
test("VocabReviewSession owns vocabulary loading, prompt, question, and SRS grading", () => {
  ...
  assert.match(source, /if \(!missingReviewEntry\) return/)
  assert.match(source, /queueMicrotask\(\(\) => \{/)
  assert.match(source, /if \(cancelled\) return/)
  assert.match(source, /dropCurrent\(\)/)
  assert.match(source, /cancelled = true/)
  assert.doesNotMatch(source, /if \(missingReviewEntry\) \{\s*dropCurrent\(\)/)
  ...
  assert.match(source, /if \(!item \|\| !question \|\| !promptModel\)/)
...
test("vocab-review missing-entry drop cancels the first Strict Mode effect so the next valid head is kept", () => {
  const source = read("src/components/review/vocab-review-session.tsx")
  const effect = source.match(/useEffect\(\(\) => \{[\s\S]*?vocabulary\.loading\]\)/)
  assert.ok(effect, "vocab-review must own a missing-entry drop effect")
  assert.match(effect[0], /if \(!currentId \|\| vocabulary\.loading \|\| vocabulary\.error\) return/)
  assert.match(effect[0], /if \(!missingReviewEntry\) return/)
  ...
  assert.doesNotMatch(effect[0], /if \(missingReviewEntry\) \{\s*dropCurrent\(\)/)
})
