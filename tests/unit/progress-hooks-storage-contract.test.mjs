...
for (const relPath of ["src/lib/kana-progress.ts", "src/lib/vocab-progress.ts"]) {
  test(`${relPath} delegates string-list persistence to progress-list-storage`, () => {
    ...
    assert.match(source, /const previousSrs = readSrsMap\(/)
    assert.match(source, /const srsSuccess =/)
    assert.match(source, /if \(!srsSuccess\) return false/)
    assert.match(source, /if \(!clearSrs\(/)
    assert.match(source, /return false/)
    assert.match(source, /return true/)
    assert.match(source, /writeSrsMap\(/)
    assert.match(source, /notifySrs\(/)
