...
for (const relPath of ["src/lib/kana-progress.ts", "src/lib/vocab-progress.ts"]) {
  test(`${relPath} delegates string-list persistence to progress-list-storage`, () => {
    ...
    assert.match(source, /readProgressListResult\(exclusionStorageKey, EXCLUSION_STORAGE_LABEL\)/)
    assert.match(source, /writeProgressList\(\s*storageKey,/)
    assert.match(source, /writeProgressList\(\s*exclusionStorageKey,/)
    if (relPath.includes("vocab-progress")) {
      ...
      assert.match(source, /const currentExcluded = readExcluded\(\)/)
      assert.match(source, /setLearnedId/)
    } else {
      ...
      assert.match(source, /const currentExcluded = readExcluded\(\)/)
      assert.match(source, /setMasteredId/)
    }
    assert.match(source, /if \(!current\.ok \|\| !currentExcluded\.ok\) return false/)
    assert.match(source, /const next = new Set\(current\.value\)/)
    assert.match(source, /const nextExcluded = new Set\(currentExcluded\.value\)/)
    assert.match(source, /const srsSuccess =/)
    assert.match(source, /return srsSuccess && writeProgressList\([\s\S]*&& writeProgressList\(/)
    assert.match(source, /expectedRaw: currentExcluded\.raw/)
    assert.match(source, /replaceInvalid: true/)
    ...
    assert.match(source, /notifyProgressList\(exclusionStorageKey\)/)
