...
test("vocabulary id registry recognizes current vocabulary ids without importing entries", () => {
  ...
  assert.equal(registry.isKnownVocabularyId("day-v-65"), true)
  assert.equal(registry.isKnownVocabularyId("day-conn-20"), true)
  assert.equal(registry.isKnownVocabularyId("day-kei-12"), true)
  assert.equal(registry.isKnownVocabularyId("flu-tech-15"), true)
  assert.equal(registry.isKnownVocabularyId("flu-conn-15"), true)
  assert.equal(registry.isKnownVocabularyId("flu-news-15"), true)
...
test("vocabulary id registry rejects stale or malformed vocabulary ids", () => {
  ...
  assert.equal(registry.isKnownVocabularyId("day-v-66"), false)
