...
test("data validation verifies PWA manifest install metadata and PNG dimensions", () => {
  ...
  assert.match(source, /\["background_color", "#d8d3cc"\]/)
  assert.match(source, /\["theme_color", "#d8d3cc"\]/)
...
test("data validation guards partitioned PWA precache drift and budgets", () => {
  ...
  assert.match(source, /PWA service worker RUNTIME_ASSETS is missing cache-worthy public assets/)
  assert.match(source, /PWA service worker runtime cache owns cache-worthy public assets/)
