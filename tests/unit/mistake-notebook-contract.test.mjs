...
test("mistake notebook hook delegates model and storage logic", () => {
  ...
  assert.match(source, /if \(!removeSrs\(MISTAKE_SRS_STORAGE_KEY, id\)\) return false/)
  assert.match(source, /if \(!clearSrs\(MISTAKE_SRS_STORAGE_KEY\)\) return false/)
  assert.match(source, /return removeSrs\(MISTAKE_SRS_STORAGE_KEY, id\)/)
