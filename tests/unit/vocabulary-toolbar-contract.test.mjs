...
test("vocabulary page delegates search, filters, and category chips to VocabularyToolbar", () => {
  ...
  assert.match(page, /const saved = await runLearningWrite\(\(\) => clearLearned\(\)\)/)
  assert.match(page, /const saved = await runLearningWrite\(\(\) => toggleLearnedId\(selectedVocab\.id\)\)/)
