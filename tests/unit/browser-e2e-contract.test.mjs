...
test("browser E2E verifies lesson progress writes after a real answer", () => {
  ...
  assert.match(e2e, /itemId === "hiragana:a"/)
  ...
  assert.match(e2e, /refresh should restore the latest correct lesson answer/)
  assert.match(e2e, /answered lesson step should remain continuable after refresh/)
  assert.match(e2e, /refresh should restore the latest wrong lesson answer/)
  assert.match(e2e, /wrong answered lesson step should remain continuable after refresh/)
  assert.match(e2e, /refreshing an answered lesson step should not duplicate practice history/)
  assert.match(e2e, /refreshing an answered lesson step should not duplicate item attempts/)
  assert.match(e2e, /refreshing an answered lesson step should not grade kana SRS again/)
  assert.match(e2e, /refreshing a wrong answered lesson step should not increment mistake count/)
  assert.match(e2e, /refreshing a wrong answered lesson step should not grade mistake SRS again/)
  ...
  assert.match(e2e, /item\.itemId === "hiragana:ha"/)
  ...
  assert.match(e2e, /completedSummary\.getByText\("こんにちは"\)/)
  ...
  assert.match(e2e, /getByTestId\("home-edit-profile"\)\.click\(\)/)
  assert.match(e2e, /home profile editor should persist a later romaji change/)
  ...
  assert.match(e2e, /srs\["hiragana:a"\]\.dueAt = now - 1/)
...
test("browser E2E verifies quiz scope controls change real question pools", () => {
  ...
  assert.match(e2e, /seionHiraganaIds\.includes\(seionPractice\.itemId\)/)
  assert.match(e2e, /!seionHiraganaIds\.includes\(allUnmasteredPractice\.itemId\)/)
...
test("browser E2E verifies filtered vocabulary exhaustion and answer freeze behavior", () => {
  const e2e = readBrowserE2ESources()

  assert.match(e2e, /async function verifyFilteredVocabularyQuizTransitions/)
  assert.match(e2e, /learnedVocab: survivalVocabIds/)
  assert.match(e2e, /all-learned vocabulary filters should show an empty state instead of falling back to the base pool/)
  assert.match(e2e, /E2E_STORAGE_KEYS\.ITEM_PROGRESS/)
  assert.match(e2e, /meaning: 30/)
  assert.match(e2e, /items\?\.\[id\]\?\.meaning >= 40/)
  assert.match(e2e, /answer-driven learning\.items updates should keep the answered question mounted/)
  assert.match(e2e, /getAttribute\("data-feedback"\)/)
  assert.match(e2e, /getByTestId\("quiz-answer-feedback"\)/)
  assert.match(e2e, /getByTestId\("quiz-next-question"\)\.click\(\)/)
  assert.match(e2e, /the exhausted filtered pool should advance only after the learner clicks next/)
})
...
test("browser E2E verifies mastered kana filters show the quiz empty state", () => {
  ...
  assert.match(e2e, /masteredKana\.includes\("hiragana:a"\)/)
  assert.match(e2e, /masteredKana\.includes\("katakana:a"\), false/)
  assert.match(e2e, /masteredKanaSrs\?\.\["hiragana:a"\]\?\.dueAt/)
  assert.match(e2e, /masteredKanaSrs\?\.\["katakana:a"\], undefined/)
  assert.match(fixtures, /export const seionRomaji = \[/)
  assert.match(fixtures, /export const seionHiraganaIds = seionRomaji\.map/)
  assert.match(e2e, /JSON\.stringify\(masteredIds\)/)
  assert.match(e2e, /getByTestId\("kana-mode-katakana"\)\.click\(\)/)
  assert.match(e2e, /katakana a should remain unmastered after mastering hiragana a/)
...
test("browser E2E verifies reference modal keyboard navigation respects focused controls", () => {
  ...
  assert.match(e2e, /keyboard\.press\("Enter"\)/)
  assert.match(e2e, /document\.activeElement\?\.getAttribute\("role"\) === "dialog"/)
  assert.match(e2e, /getByTestId\("grammar-modal-position"\)/)
  assert.match(e2e, /keyboard\.press\("ArrowRight"\)/)
  assert.match(e2e, /\^2 \\\/ \\d\+\$/)
...
test("browser E2E verifies review empty and due states", () => {
  ...
  assert.match(e2e, /reviewedKanaSrs\?\.\["hiragana:a"\]\?\.box > 1/)
  assert.match(e2e, /reviewedKanaSrs\?\.\["hiragana:a"\]\?\.right >= 1/)
  ...
  assert.ok((e2e.match(/\["meaning", "recall", "listening"\]\.includes\(item\.mode\)/g) ?? []).length >= 3)
  assert.match(e2e, /page\.context\(\)\.newPage\(\)/)
  assert.match(e2e, /unrelated cross-tab practice history should not invalidate an active review session/)
  assert.match(e2e, /active review should remain answerable after unrelated cross-tab practice/)
...
test("browser E2E verifies save failures keep quiz, lesson, and review on the current question", () => {
  ...
  assert.match(e2e, /kanaSrs\?\.\["hiragana:a"\]\?\.box, 1/)
...
test("browser E2E includes a mobile viewport smoke pass for core routes", () => {
  ...
  assert.match(e2e, /speech settings overlay should stack above navigation/)
  assert.match(e2e, /getByTestId\("speech-repeat-2"\)\.click\(\)/)
  assert.match(e2e, /E2E_STORAGE_KEYS\.SPEECH_PREFS/)
  ...
  assert.match(e2e, /getByRole\("heading", \{ name: "每天一课，循序学习" \}\)/)
...
test("browser E2E verifies non-default vocabulary levels load dynamically", () => {
  ...
  assert.match(e2e, /locator\("xpath=ancestor::article\[1\]"\)\.getByRole\("button"/)
  assert.match(e2e, /getByTestId\("vocabulary-toggle-romaji"\)/)
  assert.match(e2e, /getByText\("Mise", \{ exact: true \}\)\.waitFor\(\{ state: "hidden" \}\)/)
  ...
  assert.match(e2e, /getByTestId\("vocabulary-self-grade-hard"\)/)
  assert.match(e2e, /rapidClick\(hardSelfGrade\)/)
  assert.match(e2e, /getByTestId\("vocabulary-self-grade-status"\)/)
  assert.match(e2e, /selfAssessmentState\.results\.length, 1/)
  assert.match(e2e, /selfAssessmentResult\?\.answer, "hard"/)
  assert.match(e2e, /selfAssessmentState\.srs\?\.box, 1/)
  assert.match(e2e, /selfAssessmentState\.mistake, undefined/)
  ...
  assert.match(e2e, /getByTestId\("vocabulary-only-unlearned"\)/)
  assert.match(e2e, /getByRole\("dialog"\)\.waitFor\(\{ state: "hidden" \}\)/)
  assert.match(e2e, /mastering the focused item in the unlearned filter should persist before closing the modal/)
...
test("browser E2E verifies dynamic vocabulary load retry recovery", () => {
  ...
  assert.match(e2e, /VOCABULARY_CHUNK_ROUTE_PATTERN/)
  assert.match(e2e, /VOCABULARY_CHUNK_MARKERS/)
  assert.match(e2e, /survival:\s*\{ exportName: "survivalVocab", tokens: \['"survivalVocab"', "sur-g-1"\] \}/)
  assert.match(e2e, /daily:\s*\{ exportName: "dailyVocab", tokens: \['"dailyVocab"', "day-v-1"\] \}/)
  assert.match(e2e, /fluent:\s*\{ exportName: "fluentVocab", tokens: \['"fluentVocab"', "flu-abs-1"\] \}/)
  assert.match(e2e, /Network\.clearBrowserCache/)
  assert.match(e2e, /route\.fetch\(\)/)
  assert.match(e2e, /tokens\.every\(\(item\) => body\.includes\(item\)\)/)
  assert.ok((e2e.match(/if \(failed\)/g) ?? []).length >= 2)
  assert.match(e2e, /page\.route\(VOCABULARY_CHUNK_ROUTE_PATTERN, handler\)/)
  assert.match(e2e, /page\.unroute\(VOCABULARY_CHUNK_ROUTE_PATTERN, handler\)/)
  assert.match(e2e, /findExpressionEnd/)
  assert.match(e2e, /makeTransientFailingVocabularyChunk/)
  assert.match(e2e, /rewriteDevConstToArmedProxy/)
  assert.match(e2e, /rewriteMinifiedVocabularyChunk/)
  assert.match(e2e, /rewriteDevVocabularyChunk/)
  ...
  assert.match(e2e, /item\.itemType === "vocab"/)
  assert.match(e2e, /\["meaning", "recall", "listening"\]\.includes\(item\.mode\)/)
...
test("browser E2E verifies learning data export reset and import through the UI", () => {
  ...
  assert.match(storageKeys, /export const E2E_LEARNING_BACKUP_VERSION = 4/)
  ...
  assert.match(e2e, /unknown-key-yasashi-backup\.json/)
  assert.match(e2e, /backup files with unknown keys should be rejected instead of clearing managed learning data/)
  assert.match(e2e, /oversized-yasashi-backup\.json/)
  assert.match(e2e, /Buffer\.alloc\(2 \* 1024 \* 1024 \+ 1/)
  assert.match(e2e, /oversized learning data import should be rejected before replacing managed learning keys/)
  assert.match(e2e, /stale-yasashi-backup\.json/)
  assert.match(e2e, /version: 1/)
  assert.match(e2e, /JSON\.stringify\(\["a", "sokuon:kitte", "ka", "a"\]\)/)
  assert.match(e2e, /sokuon:kitte/)
  assert.match(e2e, /sur-g-999/)
  assert.match(e2e, /\["hiragana:a", "katakana:a", "hiragana:ka", "katakana:ka"\]/)
  assert.match(e2e, /v1 backup import should expand bare mastered kana into both scripts/)
  assert.match(e2e, /\["hiragana:a", "katakana:a", "sokuon:kitte"\]/)
  assert.match(e2e, /v1 backup import should expand bare item progress while preserving custom phonology ids/)
  assert.match(e2e, /normalizedLegacyItemProgress\["sokuon:kitte"\]\.itemId, "sokuon:kitte"/)
  assert.match(e2e, /valid backup import should remove stale vocabulary ids from learned progress/)
  assert.match(e2e, /\["hiragana:a", "katakana:a"\]/)
  assert.match(e2e, /v1 backup import should expand bare kana SRS into both scripts/)
  assert.match(e2e, /valid backup import should remove stale vocabulary ids from SRS/)
  assert.match(e2e, /selecting a valid backup should not replace managed learning keys before confirmation/)
  assert.match(e2e, /getByTestId\("learning-data-restore-dialog-cancel"\)\.click\(\)/)
  assert.match(e2e, /canceling learning data restore should keep managed learning keys/)
  assert.match(e2e, /getByTestId\("learning-data-restore-dialog-confirm"\)\.click\(\)/)
  ...
  assert.match(e2e, /selecting the exported backup should keep reset state until restore is confirmed/)
  assert.match(e2e, /keys\.every\(\(key\) => localStorage\.getItem\(key\) !== null\)/)
...
test("browser E2E text assertions reject mojibake fallbacks", () => {
  ...
  assert.match(e2e, /querySelectorAll\("path\[data-stroke-index\]"\)/)
  assert.match(e2e, /path instanceof SVGPathElement/)
  assert.match(e2e, /path\.parentElement\?\.namespaceURI !== "http:\/\/www\.w3\.org\/2000\/svg"/)
  assert.match(e2e, /path\.getTotalLength\(\) > 0/)
