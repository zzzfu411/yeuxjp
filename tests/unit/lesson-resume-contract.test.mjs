...
test("learning progress preserves compatible lesson keys while adding resume fields", () => {
  ...
  assert.match(source, /currentStepIndex\?: number/)
  assert.match(source, /lastStepId\?: string/)
  assert.match(source, /updatedAt\?: number/)
  assert.match(source, /saveLessonPosition/)
  assert.match(source, /mergeLessonProgressState\(prev\)/)
  assert.match(source, /readLessonProgressMap\(\)/)
