export type VerbConjForm = "masu" | "nai" | "te" | "ta"
...
export function conjugateVerb(dict: string, kind: VerbKind, form: VerbConjForm) {
  ...
  return godanTeOrTa(dict, "ta")
}

export function explainConjugation(verb: VerbEntry, form: VerbConjForm) {
  const label = VERB_CONJ_FORMS.find((f) => f.id === form)?.label ?? form
  ...
  if (verb.kind === "ichidan") {
    return `「${baseName}」（${verb.meaning}）的 ${label} 是「${answer}」。一段动词：去掉「る」再接「ます/ない/て/た」。`
  }

  if (verb.kind === "suru") {
    return `「${baseName}」（${verb.meaning}）的 ${label} 是「${answer}」。する动词：～します／～しない／～して／～した。`
  }

  if (verb.kind === "kuru") {
    return `「${baseName}」（${verb.meaning}）的 ${label} 是「${answer}」。くる：きます／こない／きて／きた（不规则）。`
  }
  ...
  if (form === "masu") {
    const i = GODAN_TO_I[last]
    return `「${baseName}」（${verb.meaning}）的 ${label} 是「${answer}」。五段动词：末尾「${last}」→「${i ?? "?"}」再接「ます」。`
  }

  if (form === "nai") {
    const a = GODAN_TO_A[last]
    return `「${baseName}」（${verb.meaning}）的 ${label} 是「${answer}」。五段动词：末尾「${last}」→「${a ?? "?"}」再接「ない」（う→わ）。`
  }

  if (verb.dict === "いく") {
    return `「${baseName}」（${verb.meaning}）的 ${label} 是「${answer}」。注意例外：いく → いって／いった（不是 いて／いた）。`
  }

  return `「${baseName}」（${verb.meaning}）的 ${label} 是「${answer}」。五段动词：て形/た形按词尾分组变化（う/つ/る→って/った，む/ぶ/ぬ→んで/んだ，く→いて/いた，ぐ→いで/いだ，す→して/した）。`
}
