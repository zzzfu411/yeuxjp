...
The current merge gate is `npm run check`; it includes the HTTP smoke E2E gate. The GitHub Actions workflow in `.github/workflows/quality.yml` runs this gate on push and pull request. It also has path-filtered strict gates for higher-risk browser behavior: learning-flow changes in routes, lesson/kana/vocabulary/quiz/review components, shared question/progress/SRS/mistake logic, content data, E2E harnesses, or package locks automatically provision Chromium with `e2e:install:ci` before running `npm run e2e:browser:required`; PWA-impacting changes such as service worker, manifest, offline fallback, app icons, public shell/assets, AnimCJK resources, PWA registration/navigation, PWA E2E harness, or package lock changes run `npm run e2e:pwa:required`.
