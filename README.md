...
The current merge gate is `npm run check`; it includes the HTTP smoke E2E gate. The GitHub Actions workflow in `.github/workflows/quality.yml` runs this gate on push and pull request. It also detects PWA-impacting changes, such as service worker, manifest, offline fallback, app icons, public shell/assets, AnimCJK resources, PWA registration/navigation, PWA E2E harness, or package lock changes, and automatically provisions Chromium with `e2e:install:ci` before running `npm run e2e:pwa:required`. Use `npm run check:release` for release environments that have Playwright and its Chromium browser installed; it runs the merge gate plus strict browser click-flow and PWA/offline checks. The workflow also exposes a manual `workflow_dispatch` release run that provisions Chromium plus Linux system dependencies with `e2e:install:ci` before running `check:release`. `npm run e2e` verifies HTTP route health for the core top-level pages plus the first lesson. `npm run e2e:browser` exercises real interactions with Playwright, including lesson resume persistence, lesson answer recording, kana stroke playback, vocabulary search, quiz mistakes, review queue startup, and learning data backup/restore/reset. `npm run e2e:pwa` runs against a production build, waits for the service worker, simulates offline navigation, verifies the pre-cached home page, a previously visited lesson page, cached query deep links such as `/kana?set=yoon`, a service-worker-controlled online navigation failure, and a real AnimCJK kana SVG can be served offline, and verifies the fallback page does not overwrite local learning state. Optional browser scripts exit cleanly with a skip message when the Playwright package or Chromium browser binary is missing. After `npm ci --prefix web`, run `npm run e2e:install --prefix web` to provision Chromium for local strict browser/PWA gates; in CI Linux runners use `npm run e2e:install:ci --prefix web` so Playwright can install required system dependencies too. Use the `:required` variants or `check:release` in local release checks that must fail when the browser stack is missing.
